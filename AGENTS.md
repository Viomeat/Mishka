...
**「配置变了就重启」的决策单点**：一律走 [ProxyServiceController.restartWhenReady](app/src/main/kotlin/top/yukonga/mishka/platform/ProxyServiceController.kt)，**必须读 ProxyServiceBridge 而非调用方自己的 UI 标志**——`uiState.isRunning` 在代理 Starting 窗口（约 10s）内仍为 false，据此决策会漏掉重启，表现成「界面已是新配置、代理仍跑旧的」。Starting/Stopping 过渡态不能直接重启（会与 Service 内进行中的启动协程并发），挂起等状态收敛：落到 Running 补一次，落到 Stopped/Error 放弃（用户中途手动停了就不该被拉起）。两个调用方：`onActiveSubscriptionChanged()`（切换/删除 active；删光最后一条时改为 stop + `cancelPendingRestart()`，没有配置可切时 restart 只会失败成 Error）与 `restartAfterProfileUpdate(uuid)`（见下）。重启本身是**整进程重来**而非热重载——VPN 杀进程重建 TUN fd，ROOT 另清 `runtime/{uuid}/` 沙箱与持久化 PID 让 attach 分支失效、强制全新启动读新配置。

**订阅更新后自动重启**：mihomo 只在进程启动时读一次 config.yaml，embed mode 又禁掉了 `/configs`/`/restart`，**「让更新生效」除了重启进程没有别的手段**。`RESTART_AFTER_PROFILE_UPDATE`（默认开）控制，三个更新入口成功后调 `restartAfterProfileUpdate(uuid)`：`fetchSubscription` / `updateAllSubscriptions` / `ProfileWorker.runUpdate`；编辑订阅不触发。**active 判定收在 controller 内**（读 storage），别让调用方各自判——前台手边是 DB、后台 Worker 只有 storage，两套判定源迟早不一致；非 active 订阅的更新不影响正在跑的配置，直接忽略。批量更新的重启**排在整轮结束之后且只发一次**：逐条重启会断流，后续订阅走 mixed-port 的下载会跟着一起失败。
