...
- **Pipeline 取消（当前失效，待修）**：阻塞 JNI 调用不响应协程取消，`withContext` 要等它返回才抛 `CancellationException`，此时 Go 侧 `cancelRegistry` 已清空，`nativeCancel` 成 no-op，processLock 占到 60s 超时。Go 侧设施正确，缺的是触发时机——须在阻塞调用**之前**挂 watcher 协程。`cancelCurrentUpdate` 先同步 `clearProgress()` 再 cancel。
...
CREATE → Pending ✓, Imported ∅
  → APPLY（processLock 串行）：
      ...
      ② fetchAndValid（锁外、可取消、JNI in-process）：Url 型 force=true 重新下载 config.yaml；File 型 force=false 跳过 fetch；两者均**串行** prefetch provider（每源 60s 超时、无总时限）后 Parse 校验；httpProxy 由 SubscriptionProxyResolver 决定
