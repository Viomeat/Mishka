...
│   │   │   │   ├── animation/        DampedDragAnimation（阻尼拖拽 + 按压缩放弹簧）+ InteractiveHighlight（按压高光：smoothstep shader 光斑 + 松手弹回，无 shader 时降级 radialGradient）
...
│   │   │   │   └── liquid/           iOS 液态玻璃（改编自 Kyant0/AndroidLiquidGlass，与 miuix example 同源）：LiquidGlassNavigationBar + Lens（折射/色散）+ InnerShadow + Vibrancy + CombinedBackdrop；tilt 重力高亮读取必须下沉到 drawBackdrop 的 highlight lambda（draw 阶段）+ 3° 量化，禁止组合期 `by rememberDeviceTilt()`（传感器 ~50Hz 会整栏重组）
...
- **深色判定单点**：`ThemeConfig.resolveIsDark(systemDark)` 是 colorMode→isDark 的唯一实现（App.kt 与 MainActivity 系统栏共用），组合树内一律读 `LocalAppDarkMode.current`；**屏幕/组件禁止直接 `isSystemInDarkTheme()`**，否则用户强制深/浅色时该处不跟随（AboutScreen OS3 背景 / YAML 编辑器配色曾因此混色）。主题枚举的用户可见名走 [ThemeLabels.kt](shared/src/commonMain/kotlin/top/yukonga/mishka/ui/theme/ThemeLabels.kt) 共享 label()，设置入口摘要与主题页禁止各自 when 映射。`LocalAppMonetEnabled` 标记 Monet 开关：StatusColors 仅 Running 态跟随动态取色（secondaryContainer/primary），Pending/Stopped 警示黄/红固定不随壁纸漂移。底栏毛玻璃无独立开关，跟随全局 `blurEnabled`
...
- **宽屏适配**：窗口宽度 ≥ 600dp（`WideScreenMinWidth`，`rememberIsWideScreen()`，[WindowSize.kt](shared/src/commonMain/kotlin/top/yukonga/mishka/ui/util/WindowSize.kt)；用缩放前 `LocalPlatformDensity` 量宽，界面缩放不翻转外壳）时：
